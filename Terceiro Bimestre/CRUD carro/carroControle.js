...
function procure() {
    ...
    if (isNaN(id_carro) || !Number.isInteger(Number(id_carro))) {
        mostrarAviso("Precisa ser um número inteiro");
        document.getElementById("inputId_carro").focus();
        return;
    }

    if (id_carro) { // se digitou um Id_carro
        carro = procurePorChavePrimaria(id_carro);
...
function salvar() {
    ...
    let id_carro;
    if (carro == null) {
        id_carro = parseInt(document.getElementById("inputId_carro").value);
    } else {
    ...
    if (id_carro && nome_carro && modelo_carro && ano) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                carro = new Carro(id_carro, nome_carro, modelo_carro, ano);
                listaCarro.push(carro);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                carroAlterado = new Carro(id_carro, nome_carro, modelo_carro, ano);
        ...
        leitor.onload = function (e) {
            ...
            for (let i = 0; i < linhas.length; i++) {
                ...
                if (linha) { //verifica se a linha não está vazia
                    ...
                    if (dados.length === 4) { //verifica os campos
                        // Adiciona os dados à listaCarro como um objeto
                        listaCarro.push({
                            id_carro: dados[0],
                            nome_carro: dados[1],
                            modelo_carro: dados[2],
                            ano: dados[3]
                        });
                    }
                }
